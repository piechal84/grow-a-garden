...
import type { VikingTier } from "./vikingData";
...
export interface YggdrasilState {
  ...
  x: number;
  y: number;
  /** When construction finishes (24h after planting) — the tree is inert until now >= this. */
  constructionReadyAt: number;
  /** Concurrent research capacity — starts at 1, upgradeable up to 10. */
  slots: number;
